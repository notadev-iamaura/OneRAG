...
# RAG 프롬프트 (기본값)
# 데모 경로는 운영자가 자주 바꾸는 진입점이므로 환경 변수로 외부화한다.
# 환경 변수 미설정 시 아래 한국어 기본 프롬프트를 그대로 사용한다(회귀 0).
# - DEMO_RAG_SYSTEM_PROMPT: 시스템 프롬프트 전체 문자열
# - DEMO_RAG_USER_TEMPLATE: 유저 템플릿 ({context}, {question} 자리표시자 필수)
DEFAULT_RAG_SYSTEM_PROMPT = """당신은 RAG(Retrieval-Augmented Generation) 기반 AI 어시스턴트입니다.
...
DEFAULT_RAG_USER_TEMPLATE = """다음 문서 컨텍스트를 참고하여 질문에 답변해주세요.
...
def _resolve_prompt(env_name: str, default: str) -> str:
    """환경 변수에서 프롬프트를 읽되, 미설정/공백이면 기본값으로 폴백한다."""
