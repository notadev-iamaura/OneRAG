...
def build_reflector_system_prompt(
    ...
    Args:
        ...
        template: config로 외부화된 시스템 프롬프트 템플릿. None이면 코드 내장
            기본값(REFLECTOR_SYSTEM_PROMPT_TEMPLATE)을 사용한다 → 회귀 0.

    Returns:
        출력 언어가 반영된 시스템 프롬프트 문자열
    """
    base_template = (
        template if template is not None else REFLECTOR_SYSTEM_PROMPT_TEMPLATE
    )
...
# 검색 컨텍스트가 비었을 때 프롬프트에 채우는 대체 문자열 (코드 내장 기본값=한국어)
# 운영자는 config.reflector_empty_context로 코드 포크 없이 오버라이드한다.
REFLECTOR_EMPTY_CONTEXT = "컨텍스트 없음"
...
class AgentReflector:
    ...
    def __init__(
        ...
        # config.reflector_system_prompt가 있으면 외부 템플릿을, 없으면 코드 내장
        # 기본값을 사용한다 → 미설정 시 회귀 0.
        self._system_prompt = build_reflector_system_prompt(
            getattr(config, "output_language", "한국어"),
            getattr(config, "reflector_system_prompt", None),
        )
        # 사용자 프롬프트와 빈 컨텍스트 대체 문자열도 동일하게 오버라이드 가능.
        self._user_prompt_template = (
            getattr(config, "reflector_user_prompt", None) or REFLECTOR_USER_PROMPT
        )
        self._empty_context = (
            getattr(config, "reflector_empty_context", None) or REFLECTOR_EMPTY_CONTEXT
        )
    ...
    async def reflect(
        ...
        try:
            ...
            user_prompt = self._user_prompt_template.format(
                query=query,
                answer=answer,
                context=context or self._empty_context,
            )
