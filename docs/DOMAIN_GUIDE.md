# 도메인 적용 가이드 (Domain Setup Guide) - v1.0.0

OneRAG는 코드 수정 없이 **설정 파일(YAML)**과 **데이터 적재**만으로 새로운 비즈니스 도메인에 즉시 적용할 수 있도록 설계되었습니다.
...
- **Whitelist**: 도메인 고유 명사(회사명·제품명·기관명 등)를 등록하여 PII 마스킹 오탐을 방지합니다.
...
v1.0.0부터 지원되는 **지능형 벡터 그래프 검색**을 활용하려면:
