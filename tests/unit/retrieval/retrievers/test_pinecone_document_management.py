"""
PineconeRetriever 문서 관리 메서드 단위 테스트
...
from app.modules.core.retrieval.retrievers.pinecone_retriever import PineconeRetriever


@pytest.mark.unit
class TestPineconeDocumentManagement:
    """PineconeRetriever 문서 관리 테스트"""
    ...
    @pytest.mark.asyncio
    async def test_delete_document_removes_chunks(self, retriever, mock_store):
        """문서 청크 삭제"""
        mock_store.fetch_objects.return_value = [
    ...
    async def test_get_document_details(self, retriever, mock_store):
        """문서 상세 정보"""
        mock_store.fetch_objects.return_value = [
            {"_id": "id-1", "content": "청크 내용", "document_id": "doc-1", "source_file": "r.pdf", "file_type": "PDF", "chunk_index": 0, "created_at": "2024-01-15"},
        ]

        result = await retriever.get_document_details("doc-1")
        assert result is not None
        assert result["id"] == "doc-1"
        assert result["actual_chunk_count"] == 1
    ...
    async def test_get_collection_info(self, retriever, mock_store):
        ...
        assert result["collection_name"] == "test-namespace"
