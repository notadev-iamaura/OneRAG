...
Note:
    ...
    MongoDB Atlas는 기본적으로 Dense 전용 검색 (하이브리드 미지원)
"""
...
class TestMongoDBAtlasRetrieverInit:
    """MongoDBAtlasRetriever 초기화 테스트"""

    def test_basic_initialization(
        self, mock_embedder: MockEmbedder, mock_store: MockVectorStore
    ) -> None:
        """기본 초기화 테스트"""
        from app.modules.core.retrieval.retrievers.mongodb_atlas_retriever import (
            MongoDBAtlasRetriever,
        )
    ...
    def test_initialization_with_custom_values(
        ...
        assert retriever.collection_name == "custom_docs"
...
class TestMongoDBAtlasRetrieverDenseSearch:
    """Dense 검색 테스트"""
...
class TestMongoDBAtlasRetrieverErrorHandling:
    """에러 처리 테스트"""
...
class TestMongoDBAtlasRetrieverHealthCheck:
    """health_check 테스트"""
...
class TestMongoDBAtlasRetrieverResultConversion:
    """검색 결과 변환 테스트"""
